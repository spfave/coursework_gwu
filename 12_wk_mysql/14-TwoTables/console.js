...
async function start() {
  const res = await inquirer.prompt({
    ...
    choices: [
      ...
      "Search by range",
      "Search by song",
      "Search for artist with album and songs in top chart",
      "Exit",
    ],
    name: "query",
  });
  ...
  switch (res.query) {
    ...
    case "Search by song":
      await searchBySong();
      break;
    case "Search for artist with album and songs in top chart":
      await searchByArtistTopSongAndAlbum();
      break;
...
async function searchByArtistTopSongAndAlbum() {
  const res = await inquirer.prompt([
    {
      type: "input",
      message: "Enter an artist name",
  ...
  connection.query(
    `
    SELECT albums.artist, album, albums.year, song, top5000.year
    FROM albums
    INNER JOIN top5000 ON 
    albums.artist = top5000.artist AND
    albums.year = top5000.year
    WHERE albums.artist = ?;
    `,
    [res.artist],
    (err, data) => {
